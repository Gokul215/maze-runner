...
def shortestpath(self,row,col):
        if row>len(self.mazeboard) or row <0 or col>len(self.mazeboard[0]) or col<0: return False
        self.mazeboard[row][col]='A'
        visited=[[False]*len(self.mazeboard) for i in range(len(self.mazeboard[0]))]
        directions=[(1,0),(-1,0)]
        mpath=deque()
        ...
        while len(mpath)>=1:
            ...
            for dr,dc in directions:
                ...
                c=lastpath.col+dc
                print(r,c)
                if (r<(self.mazerow)-1 and r >=0 and c<(self.mazecol)-1 and c>=0 and  not visited[r][c] ) : 
                    if  self.mazeboard[r][c] == "T" and self.mazeboard[r][c] !='M' :
